...
function editAssignLerningResultModal(button){
    var idAssignLearningResult = button.getAttribute('alr');
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'EditAssignLearningResult/getAssignLearningResultById/' + idAssignLearningResult;
...
function postPutExecution(url, dataFormALR, modalName, formModal){
    ...
    var formData = new FormData(dataFormALR);
    request.open('POST', ajaxUrl, true);
        request.send(formData);
        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status == 200){
                console.log('postput', dataFormALR);
                var objData = JSON.parse(request.responseText);
                if(objData.status){
...
function deleteAssignLearningResult(deleteButton){
    var code = deleteButton.getAttribute('alr');
    swal({
...
function searchSelect(selector){
    var selectBoxElement = document.querySelector(selector);
